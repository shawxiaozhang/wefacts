    ...
    while retry > 0:
        try:
            ...
            ftp.retrbinary('RETR %s-%d.gz' % (station_id, year),
                           open('%s/raw/%s-%d.gz' % (util.base_dir, station_id, year), 'wb').write)
            ftp.quit()
            break
        except ftplib.all_errors as e:
            retry -= 1
            if os.path.isfile('%s/raw/%s-%d.gz' % (util.base_dir, station_id, year)):
                os.remove('%s/raw/%s-%d.gz' % (util.base_dir, station_id, year))
        finally:
            if retry <= 0:
                return None

    # unzip the file
    with gzip.open('%s/raw/%s-%d.gz' % (util.base_dir, station_id, year), 'rb') as f:
        with open('%s/raw/%s-%d.txt' % (util.base_dir, station_id, year), 'w') as f2:
            file_content = f.read()
